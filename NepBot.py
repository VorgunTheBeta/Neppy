...
def on_message(message):
        if message.content.startswith('?hello'):
            ...
            f.write(msg+"\n")
            result = dbclient.files_search('/', userid, start=0,max_results=100, dropbox.files.SearchMode.filename)
            dbclient.files_upload(msg, "/"+fname, dropbox.files.WriteMode.update, client_modified=datetime.datetime.now(),mute=True)
            f.close()
